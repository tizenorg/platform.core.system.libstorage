...
static storage_ext_list *changed_list;
...
void storage_ext_release_list(storage_ext_list **list)
{
	...
	STORAGE_EXT_LIST_FOREACH(*list, dev) {
		storage_ext_release_internal(dev);
		free(dev);
	}
...
int storage_ext_get_list(storage_ext_list **list)
{
	...
	while (g_variant_iter_loop(iter, "(issssssisibii)",
		...
		STORAGE_EXT_LIST_APPEND(*list, elem);
	}
		...
		GVariant *params, gpointer user_data)
{
	...
	STORAGE_EXT_LIST_FOREACH(changed_list, callback) {
		if (!callback->func)
			continue;
...
int storage_ext_register_device_change(storage_ext_changed_cb func, void *data)
{
	...
	STORAGE_EXT_LIST_APPEND(changed_list, callback);
...
void storage_ext_unregister_device_change(storage_ext_changed_cb func)
{
	...
	STORAGE_EXT_LIST_FOREACH(changed_list, callback) {
		...
		STORAGE_EXT_LIST_REMOVE(changed_list, callback);
		free(callback);
	}
}
