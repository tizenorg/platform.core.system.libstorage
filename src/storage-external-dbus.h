...
typedef GList storage_ext_list;

#define STORAGE_EXT_LIST_FOREACH(head, node) \
	storage_ext_list *elem; \
	for (elem = head, node = NULL; \
		 elem && ((node = elem->data) != NULL); \
		 elem = g_list_next(elem), node = NULL)

#define STORAGE_EXT_LIST_APPEND(head, node) \
	head = g_list_append(head, node);

#define STORAGE_EXT_LIST_REMOVE(head, node) \
...
typedef int (*storage_ext_request_cb)(storage_ext_device *dev, int result, void *data);

void storage_ext_release_device(storage_ext_device **dev);
void storage_ext_release_list(storage_ext_list **list);
int storage_ext_get_list(storage_ext_list **list);
...
storage_ext_device *storage_ext_duplicate_device(storage_ext_device *dev);
int storage_ext_update_device(storage_ext_device *src, storage_ext_device *dst);
